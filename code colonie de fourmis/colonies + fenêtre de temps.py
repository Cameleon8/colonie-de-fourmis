...
from scoreEtudiant import instance_to_coords_and_time_list
...
def calculate_tour_length_and_penalty(tour, dist_matrix, time_list):
    """
    Calcule la longueur totale du tour (chemin) d'une fourmi.
    Le coût de la solution L_k, ainsi que l'accumulation des pénalités de temps sur le tour.
    """
    length = 0
    penalty = 0
    num_nodes = len(tour)
    for i in range(num_nodes):
        ...
        a, b = time_list[node2]
        if length < a:          #si le voyageur a de l'avance, il attend le temps que la fenêtre s'ouvre
            length += time_penalty(length, a, b)
        else:
            penalty += time_penalty(length, a, b)
    return length, penalty


def penalty_prob(current_node, eta_matrix, t, time_list):
    '''
    Met à jour les désirabilités des noeuds suivant les fenêtres de temps
    '''
    num_nodes = len(time_list)
    mu = np.zeros(num_nodes)
    for j in range(num_nodes):
        if j != current_node:
            #mu[j] = 1/(time_penalty(t + 1/eta_matrix[current_node, j], time_list[j][0], time_list[j][1]) + 1)  # pénalité que la fourmi aura si elle décide d'aller à j
            mu[j] = np.exp(-time_penalty(t+ 1/eta_matrix[current_node, j], time_list[j][0], time_list[j][1]))
    return mu
            

def calculate_prob(i, unvisited_nodes, tau_matrix, eta_matrix, mu_matrix, alpha, beta, gamma):
    """
    Calcule la probabilité p_ij d'aller du noeud i au noeud j.
    La formule est : p_ij = (tau_ij^alpha * eta_ij^beta * mu_j^gamma) / sum(tau_il^alpha * eta_il^beta * mu_l^gamma).
    """
    ...
    penalty_pow = mu_matrix[unvisited_nodes] ** gamma
    numerator = pheromones_pow * desirability_pow * penalty_pow
...
def construct_tour(start_node, num_nodes, time_list, tau_matrix, eta_matrix, alpha, beta, gamma):
    """
    ...
    while unvisited_nodes:
        ...
        mu_matrix = penalty_prob(current_node, eta_matrix, t, time_list)
        unvisited_list = list(unvisited_nodes)
        probabilities = calculate_prob(current_node, unvisited_list, tau_matrix, eta_matrix, mu_matrix, alpha, beta, gamma)

        # Sélectionner le noeud suivant de manière probabiliste
        next_node = np.random.choice(unvisited_list, 1, p=probabilities)[0]
        t += 1/eta_matrix[current_node, next_node]  #on rajoute le temps du trajet entre les noeuds "current_node" et "next_node"
...
def update_pheromones(tau_matrix, all_tours, dist_matrix, time_list, rho):
    """
    ...
    for k in range(num_ants):
        tour = all_tours[k]
        s, p = calculate_tour_length_and_penalty(tour, dist_matrix, time_list)
        x = 10 
        L_k = s + p # L_k: le coût de la solution 
...
def ACO_TSP(coords, time_list, start_node, num_ants, num_iterations, alpha, beta, gamma, rho):
    """
    ...
    for iteration in range(num_iterations):
        ...
        for ant in range(num_ants):
            tour = construct_tour(start_node, num_nodes, time_list, tau_matrix, eta_matrix, alpha, beta, gamma)
            all_tours.append(tour)

            # Évaluation et mise à jour de la meilleure solution (on privilégie la solution avec le moins de pénalité)
            length, penalty = calculate_tour_length_and_penalty(tour, dist_matrix, time_list)
            if penalty < best_tour_penalty:
                best_tour_length = length
                best_tour_penalty = penalty
                best_tour = tour
            elif penalty == best_tour_penalty and length < best_tour_length:
                best_tour_length = length
                best_tour_penalty = penalty
                best_tour = tour
        
        # 5. Mise à jour des phéromones
        tau_matrix = update_pheromones(tau_matrix, all_tours, dist_matrix, time_list, rho)

        # Affichage du progrès
        Score.append(best_tour_length)
        Penalty.append(best_tour_penalty)
   #     if (iteration + 1) % 10 == 0 or iteration == 0:
    ...
    #plt.show()
    return best_tour, best_tour_length, best_tour_penalty ,Score, Penalty

inst1 = "data/inst1"
inst2 = "data/inst2"
inst3 = "data/inst3"
cities_coords, cities_time = instance_to_coords_and_time_list(inst1)

# Paramètres de l'algorithme (à ajuster pour une performance optimale)
num_ants = 50     # Nombre de fourmis (nb_a) 
num_iterations = 100 # Nombre d'itérations
alpha = 1          # Importance des phéromones 
beta = 1           # Importance de l'heuristique (distance) 
gamma = 1         # Importance des pénalités
rho = 0            # Taux d'évaporation 
start_node = 0       # le tour se fait depuis le premier noeuds

# Exécution de l'algorithme
#best_tour, best_length, penalty, Score, Penalty = ACO_TSP(cities_coords, cities_time, start_node, num_ants, num_iterations, alpha, beta, gamma, rho)
...
#print(f"pénalité du meilleur tour: {penalty:.2f}")
#print(Penalty)
...
def courbe_score_et_penalite_intervalle_confiance(num_run, coords, time_list, start_node, num_ants, num_iterations, alpha, beta, gamma, rho):
    Score_mean = [0 for i in range(num_iterations)]
    Score_sigma = [0 for i in range(num_iterations)]
    Penalty_mean = [0 for i in range(num_iterations)]
    Penalty_sigma = [0 for i in range(num_iterations)]
    for l in range(num_run):
        S, P = ACO_TSP(coords, time_list, start_node, num_ants, num_iterations, alpha, beta, gamma, rho)[3:]
        score = S
        penalty = P
        for i in range(num_iterations):
            Score_mean[i] += score[i]
            Penalty_mean[i] += penalty[i]
    Score_Mean = [(1/num_run) * Score_mean[i] for i in range(num_iterations)]
    Penalty_Mean = [(1/num_run) * Penalty_mean[i] for i in range(num_iterations)]
    for l in range(num_run):
        score = S
        penalty = P
        for i in range(num_iterations):
            Score_sigma[i] += (score[i] - Score_Mean[i])**2
            Penalty_sigma[i] += (penalty[i] - Penalty_Mean[i])**2
    Score_Sigma = [(x/num_run)**0.5 for x in Score_sigma]
    Penalty_Sigma = [(x/num_run)**0.5 for x in Penalty_sigma]
    ...
    Penalty_inf = [Penalty_Mean[i] - Penalty_Sigma[i]/(num_run**0.5) for i in range(num_iterations)]
    Penalty_sup = [Penalty_Mean[i] + Penalty_Sigma[i]/(num_run**0.5) for i in range(num_iterations)]
    ...
    plt.title("Evolution du score selon le nombre d'itération ")
    ...
    plt.show()
      ...
      # Dessiner la courbe de tendance principale
    plt.plot(range(num_iterations), Penalty_Mean, color='darkblue', linewidth=2, label='Pénalité')

    # Utiliser fill_between pour dessiner l'intervalle de confiance
    plt.fill_between(range(num_iterations), Penalty_inf, Penalty_sup, color='skyblue', alpha=0.5)
    

     # Ajouter des étiquettes et un titre
    plt.title("Evolution de la pénalité selon le nombre d'itération ")
    plt.xlabel("nombre d'itération")
    plt.ylabel("Pénalité")
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
...
num_run = 1 #nombre de test pour générer des intervalles de confiance

courbe_score_et_penalite_intervalle_confiance(num_run, cities_coords, cities_time, start_node, num_ants, num_iterations, alpha, beta, gamma, rho )
